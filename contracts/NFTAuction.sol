...
contract NFTAuction is ERC721URIStorage{
    ...
    // mint a new NFT
    function mintNFT(string memory _name, string memory  _tokenURI, uint256 _price) external payable notZeroAddress {
        // increment counter
        NFTCounter++;
    ...
    // get total number of tokens owned by an address,用于查询指定地址的 NFT 总数
    function getTotalNumberOfTokensOwnedByAnAddress(address _owner) public view returns(uint256) {
        uint256 totalNumberOfTokensOwned = balanceOf(_owner);
        return totalNumberOfTokensOwned;
    }

    //定义一个公共函数，用于开始一个 NFT 的拍卖，传入参数为 NFT 的 ID，最低出价和拍卖持续时间  
    ...
    function claimNFT(uint256 _tokenID) public payable tokenExist(_tokenID) notZeroAddress returns (bool success) {
        ...
        balances[tokenOwner] += auction.highestBid;
        balances[msg.sender] -= auction.highestBid;
        // // get owner of the token
      ...
      function recharge(address _account) public payable{    //充值代币
        //totalSupply += _initialSupply;
        balances[_account] += msg.value;
        account=_account;
    }
       function ETHbalanceOf(address _account) public view returns (uint256) {    //查询账户的余额
